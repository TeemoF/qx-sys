...
<script>
			...
			columns : [ [
					...
					},
					{
						...
						formatter : function(value, user, index) {
							...
							//return '<button type="button" onclick="detail('+index+')" class="btn btn-link">'+user.nickname+'</button>';
						}
					},
					{
						field : 'email',
					...
					},
					{
						...
						formatter : function(value, user, index) {
							...
							$(user.roles).each(function(index,role){
								str += role.roleName+(index==(user.roles.length-1)?"":",");
							});
					...
					},
					{
						...
						width:150,
						formatter:function(value, user, index){
							return '<i class="glyphicon glyphicon-share-alt"></i><a href="javascript:selectPermissionById('+index+');">选择角色</a>';
						}
					}
	...
	*/
	function selectPermissionById(index){
		...
		$.post("${basePath}/getRoles",{userid:row.id},function(result){
			$('#dg').datagrid('clearSelections');
			if(result && result.length){
				console.log(result);
				var html =[];
				html.push('<div class="checkbox"><label><input type="hidden" name="userid" value="'+row.id+'"/><input type="checkbox"  name="selectAllBox">全选</label></div>');
				...
				return layer.msg('没有获取到角色数据，请先添加角色数据。',1500);
			}
		},'json');
	}
